...
{
public:
	EllipseDistanceFunctor(RS_Ellipse* ellipse, LDOUBLE const& target) : distance(target)
	{ // Constructor
		e=ellipse;
		ra=e->getMajorRadius();
		k2=1.- e->getRatio()*e->getRatio();
	}
	void setDistance(const LDOUBLE& target){
		distance=target;
	}
#if BOOST_VERSION > 104500
	boost::math::tuple<LDOUBLE, LDOUBLE, LDOUBLE> operator()(LDOUBLE const& z) const {
#else
	boost::fusion::tuple<LDOUBLE, LDOUBLE, LDOUBLE> operator()(LDOUBLE const& z) const {
#endif
	LDOUBLE cz=cos(z);
	LDOUBLE sz=sin(z);
		//delta amplitude
	LDOUBLE d=sqrtl(1-k2*sz*sz);
...
RS_VectorSolutions RS_Ellipse::getFoci() const {
	...
	if(getRatio()>1.)
		e.switchMajorMinor();
	RS_Vector vp(e.getMajorP()*sqrtl(1.-e.getRatio()*e.getRatio()));
...
RS_Vector RS_Ellipse::getNearestEndpoint(const RS_Vector& coord, LDOUBLE* dist)const {
	LDOUBLE dist1, dist2;
    RS_Vector startpoint = getStartpoint();
    RS_Vector endpoint = getEndpoint();
...
/**
  ...
  * \author: Dongxu Li
  */
LDOUBLE RS_Ellipse::getLength() const
{
...
* \author Dongxu Li
**/
LDOUBLE RS_Ellipse::getEllipseLength(LDOUBLE x1, LDOUBLE x2) const
{
	LDOUBLE a(getMajorRadius()),k(getRatio());
...
//        std::cout<<"Adding "<<ret<<" of E("<<k<<")\n";
		ret*=boost::math::ellint_2<LDOUBLE>(k);
    } else {
        ret=0.;
    }
    x1=fmod(x1,M_PI);
    x2=fmod(x2,M_PI);
	if( fabsl(x2-x1)>RS_TOLERANCE_ANGLE)  {
...
/**
  ...
  */
LDOUBLE RS_Ellipse::getEllipseLength( LDOUBLE x2) const
{
...
RS_Vector RS_Ellipse::getNearestDist(LDOUBLE distance,
...
//    RS_DEBUG->print("RS_Ellipse::getNearestDist() begin\n");
	...
	RS_Ellipse e(nullptr,data);
    if(e.getRatio()>1.) e.switchMajorMinor();
	LDOUBLE ra=e.getMajorRadius();
	LDOUBLE rb=e.getRatio()*ra;
    if(e.isReversed()) {
        std::swap(e.data.angle1,e.data.angle2);
        e.setReversed(false);
    }
	...
	LDOUBLE x1=e.getAngle1();
	LDOUBLE x2=e.getAngle2();
    if(x2<x1+RS_TOLERANCE_ANGLE) x2 += 2.*M_PI;
	LDOUBLE l=e.getEllipseLength(x1,x2); // the getEllipseLength() function only defined for proper e
	...
	LDOUBLE guess= distance*(ra+rb)/(2.*ra*rb);
	...
	int digits=std::numeric_limits<LDOUBLE>::digits;
...
//std::cout<<"RS_Ellipse::getNearestDist() dist="<<distance<<" out of total="<<l<<std::endl;
	RS_Vector vp1(e.getEllipsePoint(boost::math::tools::halley_iterate<EllipseDistanceFunctor,LDOUBLE>(
                                      X, guess, x1, x2, digits)));
    X.setDistance(l-distance);
    guess=x1+(x2-guess);
	RS_Vector vp2(e.getEllipsePoint(boost::math::tools::halley_iterate<EllipseDistanceFunctor,LDOUBLE>(
                                      X, guess, x1, x2, digits)));
    x1= (vp1-coord).squared();
    x2= (vp2-coord).squared();
    if( x1 > x2 ){
		if(dist !=nullptr)  *dist=sqrtl(x2);
        return vp2;
    }else{
		if(dist !=nullptr)  *dist=sqrtl(x1);
        return vp1;
    }
}
...
{
	if (fabsl(data.ratio) < RS_TOLERANCE) return false;
 ...
 */
RS_Vector  RS_Ellipse::getEllipsePoint(const LDOUBLE& a) const {
    RS_Vector p(a);
	LDOUBLE ra=getMajorRadius();
		...
		bool onEntity, LDOUBLE* dist, RS_Entity** entity)const
{
    ...
    ret.move(-getCenter());
    ret.rotate(-getAngle());
	LDOUBLE x=ret.x,y=ret.y;
	LDOUBLE a=getMajorRadius();
	LDOUBLE b=a*getRatio();
    //std::cout<<"(a= "<<a<<" b= "<<b<<" x= "<<x<<" y= "<<y<<" )\n";
    //std::cout<<"finding minimum for ("<<x<<"-"<<a<<"*cos(t))^2+("<<y<<"-"<<b<<"*sin(t))^2\n";
	LDOUBLE twoa2b2=2*(a*a-b*b);
	LDOUBLE twoax=2*a*x;
	LDOUBLE twoby=2*b*y;
	LDOUBLE a0=twoa2b2*twoa2b2;
	std::vector<LDOUBLE> ce(4,0.);
    ...
    if(a0 > RS_TOLERANCE2 ) { // a != b , ellipse
        ...
        a0=twoby/twoax;
		roots.push_back(sqrtl(1./(1.+a0*a0)));
...
bool RS_Ellipse::isPointOnEntity(const RS_Vector& coord,
	...
	LDOUBLE t=fabsl(tolerance);
	LDOUBLE a=getMajorRadius();
	LDOUBLE b=a*getRatio();
    RS_Vector vp((coord - getCenter()).rotate(-getAngle()));
    if ( a<RS_TOLERANCE ) {
        //radius treated as zero
		if(fabsl(vp.x)<RS_TOLERANCE && fabsl(vp.y) < b) return true;
        return false;
    }
    if ( b<RS_TOLERANCE ) {
        //radius treated as zero
		if (fabsl(vp.y)<RS_TOLERANCE && fabsl(vp.x) < a) return true;
        return false;
    }
    vp.scale(RS_Vector(1./a,1./b));

	if (fabsl(vp.squared()-1.) > t) return false;
...
RS_Vector RS_Ellipse::getNearestCenter(const RS_Vector& coord,
	...
	LDOUBLE      distCenter = coord.distanceTo(data.center);
    ...
    if( 2 == vsFoci.getNumber()) {
		...
		LDOUBLE distFocus1 = coord.distanceTo(vFocus1);
		LDOUBLE distFocus2 = coord.distanceTo(vFocus2);
...
bool	RS_Ellipse::createFrom4P(const RS_VectorSolutions& sol)
{
	...
	std::vector<std::vector<LDOUBLE> > mt;
	...
	if(fabsl(dn[0])<RS_TOLERANCE15
			||fabsl(dn[2])<RS_TOLERANCE15
...
bool	RS_Ellipse::createFromCenter3Points(const RS_VectorSolutions& sol) {
    ...
    case 2:
		...
		setMajorP(RS_Vector(1./sqrtl(dn[0]),0.));
		setRatio(sqrtl(dn[0]/dn[1]));
        setAngle1(0.);
        setAngle2(0.);
        setCenter(sol.get(0));
        return true;
...
/** \brief create from quadratic form:
  ...
  *@author: Dongxu Li
  */
bool RS_Ellipse::createFromQuadratic(const std::vector<LDOUBLE>& dn){
	RS_DEBUG->print("RS_Ellipse::createFromQuadratic() begin\n");
	if(dn.size()!=3) return false;
//	if(fabsl(dn[0]) <RS_TOLERANCE2 || fabsl(dn[2])<RS_TOLERANCE2) return false; //invalid quadratic form

	//eigenvalues and eigenvectors of quadratic form
    // (dn[0] 0.5*dn[1])
	// (0.5*dn[1] dn[2])
	LDOUBLE a=dn[0];
	const LDOUBLE c=dn[1];
	LDOUBLE b=dn[2];

	//Eigen system
	const LDOUBLE d = a - b;
	const LDOUBLE s=sqrtl(d*d + c*c);
	// { a>b, d>0
	...
	if(a>=b) {
		setMajorP(RS_Vector(atan2l(d+s, -c))/sqrtl(0.5L*(a+b-s)));
	}else{
		setMajorP(RS_Vector(atan2l(-c, s-d))/sqrtl(0.5L*(a+b-s)));
	}
	setRatio(sqrtl((a+b-s)/(a+b+s)));
...
bool RS_Ellipse::createFromQuadratic(const LC_Quadratic& q){
	...
	LDOUBLE const& a=mQ(0,0);
	LDOUBLE const& c=2.*mQ(0,1);
	LDOUBLE const& b=mQ(1,1);
	auto  const& mL=q.getLinear();
	LDOUBLE const& d=mL(0);
	LDOUBLE const& e=mL(1);
	LDOUBLE determinant=c*c-4.*a*b;
	...
	const LDOUBLE factor=-1./qCentered.m_dConst;
	//quadratic terms
...
bool	RS_Ellipse::createInscribeQuadrilateral(const std::vector<RS_Line*>& lines)
{
	...
	if(parallel==1){
		...
		if( fabsl(centerPoint.distanceTo(l0->getStartpoint()) - centerPoint.distanceTo(l0->getEndpoint()))>RS_TOLERANCE)
			return false;
		//symmetric
		RS_DEBUG->print("RS_Ellipse::createInscribeQuadrilateral(): symmetric trapezoid detected\n");
		LDOUBLE d=l0->getDistanceToPoint(centerPoint);
		LDOUBLE l=((l0->getLength()+l1->getLength()))*0.25;
		LDOUBLE k= 4.*d/fabsl(l0->getLength()-l1->getLength());
		LDOUBLE theta=d/(l*k);
	...
	for(const RS_Vector& vp: tangent){
		...
		std::vector<LDOUBLE> mtRow;
		mtRow.push_back(vp.x*vp.x);
		mtRow.push_back(vp.x*vp.y);
		mtRow.push_back(vp.y*vp.y);
		const LDOUBLE l=sqrtl(mtRow[0]*mtRow[0]+mtRow[1]*mtRow[1]+mtRow[2]*mtRow[2]);
		bool addRow(true);
		for(const auto& v: mt){
			const LDOUBLE dx=v[0] - mtRow[0];
			const LDOUBLE dy=v[1] - mtRow[1];
			const LDOUBLE dz=v[2] - mtRow[2];
			if( sqrtl(dx*dx + dy*dy + dz*dz) < symTolerance*l){
				//symmetric
				addRow=false;
				break;
			}
	...
	case 2:{// the quadrilateral is a parallelogram
		...
		//        LDOUBLE angle(center.angleTo(tangent[0]));
		RS_Vector majorP(tangent[0]);
		LDOUBLE dx(majorP.magnitude());
		...
		LDOUBLE dy2(minorP.squared());
		if(fabsl(minorP.y)<RS_TOLERANCE || dy2<RS_TOLERANCE2) return false; //refuse to return zero size ellipse
		// y'= y
		// x'= x-y/tan
		// reverse scale
		// y=y'
		...
		//
		LDOUBLE ia2=1./(dx*dx);
		LDOUBLE ib2=1./(minorP.y*minorP.y);
...
RS_Vector RS_Ellipse::getNearestMiddle(const RS_Vector& coord,
	...
	LDOUBLE amin=getCenter().angleTo(getStartpoint());
	LDOUBLE amax=getCenter().angleTo(getEndpoint());
    if(isReversed()) {
        std::swap(amin,amax);
    }
										...
										bool onEntity ) const
{
    ...
    RS_Vector aV(-getAngle());
    direction.rotate(aV);
	LDOUBLE angle=direction.scale(RS_Vector(1.,getRatio())).angle();
	LDOUBLE ra(getMajorRadius());
...
void RS_Ellipse::rotate(const RS_Vector& center, const LDOUBLE& angle) {
...
void RS_Ellipse::rotate( const LDOUBLE& angle) {//rotate around origin
    RS_Vector aV(angle);
...
void RS_Ellipse::correctAngles() {
        ...
        *pa2 = *pa1 + fmod(*pa2 - *pa1, 2.*M_PI);
		if ( fabsl(data.angle1 - data.angle2) < RS_TOLERANCE_ANGLE ) *pa2 += 2.*M_PI;
}
...
RS2::Ending RS_Ellipse::getTrimPoint(const RS_Vector& trimCoord,
	...
	//LDOUBLE angEl = getEllipseAngle(trimPoint);
	LDOUBLE angM = getEllipseAngle(trimCoord);
...
//special trimming for ellipse arc
    ...
    if(RS_Math::isSameDirection(getAngle1(),getAngle2(),RS_TOLERANCE_ANGLE)
		...
		LDOUBLE dia=fabsl(remainder(ia-am,2*M_PI));
		LDOUBLE dia2=fabsl(remainder(ia2-am,2*M_PI));
...
LDOUBLE RS_Ellipse::getEllipseAngle(const RS_Vector& pos) const {
...
void RS_Ellipse::scale(const RS_Vector& center, const RS_Vector& factor) {
	...
	LDOUBLE a(vp1.magnitude());
    if(a<RS_TOLERANCE) return; //ellipse too small
    vp1 *= 1./a;
	LDOUBLE ct=vp1.x;
	LDOUBLE ct2 = ct*ct; // cos^2 angle
	LDOUBLE st=vp1.y;
	LDOUBLE st2=1.0 - ct2; // sin^2 angle
	LDOUBLE kx2= factor.x * factor.x;
	LDOUBLE ky2= factor.y * factor.y;
//    LDOUBLE a=getMajorRadius();
	LDOUBLE b=getRatio()*a;
	LDOUBLE cA=0.5*a*a*(kx2*ct2+ky2*st2);
	LDOUBLE cB=0.5*b*b*(kx2*st2+ky2*ct2);
	LDOUBLE cC=a*b*ct*st*(ky2-kx2);
    if (factor.x < 0)
...
LDOUBLE RS_Ellipse::getDirection1() const {
    RS_Vector vp;
    if (isReversed()){
...
LDOUBLE RS_Ellipse::getDirection2() const {
    RS_Vector vp;
    if (isReversed()){
...
void RS_Ellipse::moveRef(const RS_Vector& ref, const RS_Vector& offset) {
    ...
    for(size_t i=0; i< 2 ; i++){
        if ((ref-foci.at(i)).squared()<1.0e-8) {
			...
			LDOUBLE d=getMajorP().magnitude();
			LDOUBLE c=0.5*focusNew.distanceTo(foci.at(1-i));
			LDOUBLE k=majorP.magnitude();
    ...
    if ((ref-getMajorPoint()).squared()<1.0e-8) {
		...
		LDOUBLE r=majorP.magnitude();
        if(r<RS_TOLERANCE) return;
		LDOUBLE ratio = getRatio()*getMajorRadius()/r;
        setMajorP(majorP);
        setRatio(ratio);
        return;
    }
    if ((ref-getMinorPoint()).squared()<1.0e-8) {
		...
		LDOUBLE r2=getMajorP().squared();
        if(r2<RS_TOLERANCE2) return;
         RS_Vector projected= getCenter() +
                getMajorP()*getMajorP().dotP(minorP-getCenter())/r2;
		LDOUBLE r=(minorP - projected).magnitude();
        if(r<RS_TOLERANCE) return;
		LDOUBLE ratio = getRatio()*r/getMinorRadius();
        setRatio(ratio);
        return;
    }
 ...
 *@author Dongxu Li
 */
LDOUBLE RS_Ellipse::areaLineIntegral() const
{
	const LDOUBLE a=getMajorRadius();
	const LDOUBLE b=getMinorRadius();
    if(!isArc())
        return M_PI*a*b;
	const LDOUBLE ab=a*b;
	const LDOUBLE r2=a*a+b*b;
	const LDOUBLE& cx=data.center.x;
	const LDOUBLE aE=getAngle();
	const LDOUBLE& a0=data.angle1;
	const LDOUBLE& a1=data.angle2;
	const LDOUBLE fStart=cx*getStartpoint().y+0.25*r2*sin(2.*aE)*cos(a0)*cos(a0)-0.25*ab*(2.*sin(aE)*sin(aE)*sin(2.*a0)-sin(2.*a0));
	const LDOUBLE fEnd=cx*getEndpoint().y+0.25*r2*sin(2.*aE)*cos(a1)*cos(a1)-0.25*ab*(2.*sin(aE)*sin(aE)*sin(2.*a1)-sin(2.*a1));
...
LDOUBLE RS_Ellipse::getAngle() const {
	return data.majorP.angle();
}

LDOUBLE RS_Ellipse::getAngle1() const {
	return data.angle1;
}

void RS_Ellipse::setAngle1(LDOUBLE a1) {
	data.angle1 = a1;
}

LDOUBLE RS_Ellipse::getAngle2() const {
	return data.angle2;
}

void RS_Ellipse::setAngle2(LDOUBLE a2) {
	data.angle2 = a2;
}
...
LDOUBLE RS_Ellipse::getRatio() const {
	return data.ratio;
}

void RS_Ellipse::setRatio(LDOUBLE r) {
	data.ratio = r;
}

LDOUBLE RS_Ellipse::getAngleLength() const {
	LDOUBLE ret;
    if (isReversed()) {
...
LDOUBLE RS_Ellipse::getMajorRadius() const {
...
LDOUBLE RS_Ellipse::getMinorRadius() const {
	return data.majorP.magnitude()*data.ratio;
}

void RS_Ellipse::draw(RS_Painter* painter, RS_GraphicView* view, LDOUBLE& patternOffset) {
    if(isArc()==false){
        RS_Ellipse arc(*this);
...
void RS_Ellipse::drawVisible(RS_Painter* painter, RS_GraphicView* view, LDOUBLE& /*patternOffset*/) {
...
//    std::cout<<*this<<std::endl;
    ...
    if(pat->num>0){
		double dpmm=(double) static_cast<RS_PainterQt*>(painter)->getDpmm();
        while( i<pat->num){
			ds[i]= (double) dpmm * pat->pattern[i] ;//pattern length
			if(fabs(ds[i])<1.)
                ds[i]=(ds[i]>=0.)?1.:-1.;
			++i;
        }
